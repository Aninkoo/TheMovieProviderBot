...
async def start(client, message):
    if OWNER == message.from_user.id:
        await verify_user(client, message.from_user.id, 2345)
    ...
    if data.split("-", 1)[0] == "BATCH":
                ...
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            ...
                            [
                                InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=f'https://t.me/paxmovies') #Don't change anything without contacting me @LazyDeveloperr
                            ]
                        ]
                    )
                )
        ...
        if str(message.from_user.id) != str(userid):
            return await message.reply_text(
                text="<b>Invalid link or Expired link ! \n\n⬆️ Make sure to use the last Verification Button above ⬆️</b>",
